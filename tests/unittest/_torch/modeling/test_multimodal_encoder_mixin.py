...
def test_setup_attn_metadata_builds_with_engine_sizes():
    """A token budget above the encoder fallback passes through unchanged."""
    encoder = _DummyEncoder()
    big = _ENCODER_FALLBACK_MAX_NUM_REQUESTS + 100
    encoder.setup_attn_metadata(max_num_tokens=big)
...
def test_setup_attn_metadata_floors_small_token_budget():
    """A token budget below the legacy request fallback is floored to it."""
    encoder = _DummyEncoder()
    encoder.setup_attn_metadata(max_num_tokens=100)
...
def test_setup_attn_metadata_is_idempotent_per_call():
    ...
    encoder.setup_attn_metadata(max_num_tokens=200)
...
def test_pixtral_conservatively_allows_one_context_per_token():
...
def test_subclass_can_override_setup_attn_metadata():
    ...
    class _OverridingEncoder(nn.Module, MultimodalEncoderMixin):
        ...
        def setup_attn_metadata(self, max_num_tokens):
    ...
    encoder.setup_attn_metadata(max_num_tokens=50)

    assert encoder.attn_metadata is None  # default impl not invoked
    assert captured == {"max_num_tokens": 50}
