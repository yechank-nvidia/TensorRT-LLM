...
- **Multimodal embeddings cache** is a per-model, cross-request LRU cache of encoder embeddings.
  ...
  through the item path populate the cache for later requests. When combined with side-stream
  prefetch, peak memory is the cache capacity plus any in-flight prefetched encoder inputs and
  outputs.
