...
class HCXVisionInputProcessor(InputProcessor):
    ...
    def __init__(self, model_path: str, model_config: PretrainedConfig,
        ...
        model = HFAutoModelForCausalLM.from_pretrained(model_path,
                                                       trust_remote_code=True)
        self.device = 'cuda'
    ...
    def _post_init(self):
        possible_resolutions = []
        if self.model_config.anyres:
            assert self.model_config.max_num_grids > 0
            for i in range(1, self.model_config.max_num_grids + 1):
                for j in range(1, self.model_config.max_num_grids + 1):
                    if i == 1 and j == 1 and not self.model_config.use_1x1_grid:
                        continue
                    if i * j <= self.model_config.max_num_grids:
                        possible_resolutions.append([i, j])

            possible_resolutions = [[
                ys * self.model_config.vision_config["image_size"],
                xs * self.model_config.vision_config["image_size"]
    ...
    def _post_process(self, input_ids: torch.Tensor,
        ...
        for batch_idx, sample in enumerate(input_ids):
            ...
            assert len(img_start_ids) == image_cnts[batch_idx] == len(
        ...
        mm_features = torch.cat(list(chain(*image_features)), dim=0).to('cpu')
        return fused_input_ids[0], mm_features

    @nvtx_range("Inputprocessor _process()")
    def _process(self, preprocessed_image: dict[str, any]):

        pixel_values = preprocessed_image.get("pixel_values", None)
        pixel_values = [[
            pixel_value.to('cuda') for pixel_value in pixel_values_inner
        ] for pixel_values_inner in pixel_values]
        ...
        visual_token_idx = 0 if "siglip" in self.model_config.vision_config[
            "model_type"] else 1
        ...
        for i in range(n_chunks):
            ...
            if self.model_config.use_nth_layer == -1:
                ...
                outs = outs.hidden_states[
                    self.model_config.use_nth_layer][:, visual_token_idx:]
    ...
    @nvtx_range("Inputprocessor _preprocess()")
    ...
    def __call__(
        ...
        image_features = self._process(preprocessed_image)

        fused_input_ids, mm_features = self._post_process(
            input_ids, image_features, preprocessed_image)
        # SHAPE: (423, 3072) => 1,299,456 / linearly increasing with input_length + image_cnts + input_ids
