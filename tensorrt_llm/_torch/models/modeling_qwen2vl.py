...
class Qwen2VLInputProcessorBase(BaseMultimodalInputProcessor,
    ...
    def __init__(self,
        ...
        self._processor_artifacts_ready: OrderedDict[Hashable, Tuple[
            Any, int]] = OrderedDict()
    ...
    def set_processor_artifact_cache_max_bytes(self, max_bytes: int) -> None:
        ...
        with self._processor_artifacts_lock:
            ...
            while (self._processor_artifacts_ready
                   and self._processor_artifacts_ready_bytes > max_bytes):
                _, (_,
                    artifact_bytes) = self._processor_artifacts_ready.popitem(
                        last=False)
    ...
    def _vision_processor_artifact_bytes(artifact) -> int:
        ...
        for value in artifact.values():
            if not isinstance(value,
                              torch.Tensor) or value.device.type != "cpu":
                continue
    ...
    def _cache_vision_processor_artifact(self, artifact_key: Hashable,
        ...
        with self._processor_artifacts_lock:
            ...
            while (self._processor_artifacts_ready
                   and self._processor_artifacts_ready_bytes + artifact_bytes
                   > max_bytes):
                _, (_, evicted_bytes) = self._processor_artifacts_ready.popitem(
                    last=False)
                self._processor_artifacts_ready_bytes -= evicted_bytes
            self._processor_artifacts_ready[artifact_key] = (artifact,
                                                             artifact_bytes)
            self._processor_artifacts_ready_bytes += artifact_bytes
        ...
        **kwargs,
        ...
        try:
            ...
            if not future.done():
                future.cancel()
            with self._processor_artifacts_lock:
                if self._processor_artifacts_in_flight.get(
                        artifact_key) is future:
