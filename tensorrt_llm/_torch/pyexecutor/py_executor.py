...
class PyExecutor:
    ...
    def __init__(
        ...
        self._mm_encoder_item_scheduling_model_enabled = getattr(
            model_engine, "mm_encoder_item_scheduling_model_enabled",
            ...
            while True:
                ...
                if self.dist.rank != 0:
                    ...
                    if isinstance(self.scheduler, MultimodalScheduler):
                        local_scheduler_output = self.scheduler.replay_request(
                            ...
                            blocked_request_ids=(
                                scheduled_batch.mm_encoder_blocked_request_ids),
                            scheduled_items=(
                ...
                if self._mm_encoder_item_scheduling_enabled:
                    mm_encoder_result = self._forward_multimodal_encoder_step(
                      ...
                      and self._mm_encoder_item_scheduling_model_enabled):
                    for request in scheduled_batch.context_requests:
                        if request.py_multimodal_data is not None:
                            strip_mm_encoder_inputs(request.py_multimodal_data)
                if self._mm_encoder_item_scheduling_model_enabled:
        ...
        def _respond_if_invalid(request: LlmRequest) -> bool:
            ...
            try:
                ...
                if self._mm_encoder_item_scheduling_model_enabled:
                    ...
                    if not self._mm_encoder_item_scheduling_enabled:
                        # Every PP stage validates the same request contract,
                        # but only PP0 retains item references and cache state.
    ...
    def _schedule(self):
        ...
        replay_mm_delta = (self._mm_encoder_item_scheduling_enabled
                           and self.dist.pp_size == 1
                           and not self.enable_attention_dp
                           and (self.dist.tp_size > 1 or self.dist.cp_size > 1))
        scheduler_output = None
        if not replay_mm_delta or self._is_mm_encoder_cache_authority():
        ...
        if self._pending_mm_encoder_cache_removals:
            ...
            scheduler_output = scheduler_output._replace(
                mm_encoder_cache_removals=(
                    ...
                    list(scheduler_output.mm_encoder_cache_removals or ())))
            self._pending_mm_encoder_cache_removals = []
        ...
        if replay_mm_delta:
            ...
            if self.dist.tp_size > 1:
                mm_delta = self.dist.tp_broadcast(mm_delta, root=0)
            if self.dist.cp_size > 1:
                mm_delta = self.dist.cp_broadcast(mm_delta, root=0)
            if not self._is_mm_encoder_cache_authority():
                if not isinstance(self.scheduler, MultimodalScheduler):
                    raise RuntimeError(
                        "MM cache follower has no multimodal scheduler replay path"
                    )
                blocked_request_ids, scheduled_items, cache_removals = mm_delta
                scheduler_output = self.scheduler.replay_request(
                    self.active_requests,
                    self.inflight_req_ids,
        ...
        scheduled_requests.scheduled_mm_encoder_items = (
            scheduler_output.scheduled_mm_encoder_items)
        scheduled_requests.mm_encoder_blocked_request_ids = (
            scheduler_output.mm_encoder_blocked_request_ids)
    ...
    def _handle_multimodal_encoder_request_error(
        ...
        """Remove the same failed MM dependents on every participating rank."""
        failed_requests = [
            request for request in self.active_requests
        ...
        if self.enable_attention_dp and self.dist.world_size != 1:
            # `_handle_errors` enqueues responses through a TP collective, but
            # attention-DP ranks schedule independent requests. Perform local
    ...
    def _is_mm_encoder_cache_authority(self) -> bool:
        return (self._mm_encoder_item_scheduling_enabled
                and (self.enable_attention_dp or self.global_rank == 0))
